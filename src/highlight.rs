...
fn keywords_for(lang: Language) -> &'static [&'static str] {
    match lang {
        ...
        Language::Bash => &[
            ...
            "export", "readonly", "declare", "typeset", "unset", "shift", "break",
            "continue", "exit", "trap", "exec", "eval", "source", "set", "unset",
            "true", "false",
        ],
        Language::Sql => &[
            "SELECT", "FROM", "WHERE", "JOIN", "LEFT", "RIGHT", "INNER", "OUTER",
            "FULL", "CROSS", "ON", "AS", "INSERT", "INTO", "VALUES", "UPDATE",
            "SET", "DELETE", "CREATE", "TABLE", "VIEW", "INDEX", "DROP", "ALTER",
            "ADD", "COLUMN", "PRIMARY", "KEY", "FOREIGN", "REFERENCES", "UNIQUE",
            "NOT", "NULL", "DEFAULT", "CONSTRAINT", "CHECK", "AND", "OR", "IN",
            "EXISTS", "BETWEEN", "LIKE", "IS", "DISTINCT", "ORDER", "BY", "GROUP",
            "HAVING", "LIMIT", "OFFSET", "UNION", "ALL", "EXCEPT", "INTERSECT",
            "CASE", "WHEN", "THEN", "ELSE", "END", "BEGIN", "COMMIT", "ROLLBACK",
            "TRANSACTION", "WITH", "RECURSIVE",
            // lowercase variants
            "select", "from", "where", "join", "left", "right", "inner", "outer",
            "full", "cross", "on", "as", "insert", "into", "values", "update",
            "set", "delete", "create", "table", "view", "index", "drop", "alter",
        ...
        Language::GraphQl => &[
            ...
            "interface", "union", "enum", "input", "schema", "directive",
            "extend", "implements", "scalar", "true", "false", "null",
            "repeatable",
        ],
        Language::Auto => &[],
    }
...
fn builtins_for(lang: Language) -> &'static [&'static str] {
    match lang {
        Language::Rust => &[
            ...
            "debug_assert_ne", "todo", "unimplemented", "unreachable",
            "vec", "format", "write", "writeln", "concat", "include",
            "include_str", "include_bytes", "stringify", "env", "cfg",
            "option_env", "file", "line", "column", "module_path",
        ...
        Language::TypeScript | Language::JavaScript => &[
            ...
            "Promise", "Symbol", "Proxy", "Reflect", "Intl",
        ...
        Language::Bash => &[
            ...
            "cp", "mv", "chmod", "chown", "grep", "sed", "awk", "cut",
            "sort", "uniq", "wc", "head", "tail", "cat", "less", "more",
            "find", "xargs", "curl", "wget", "ssh", "scp", "git",
            "make", "sudo", "su", "which", "type", "alias",
        ],
        Language::Sql => &[
            "COUNT", "SUM", "AVG", "MAX", "MIN", "COALESCE", "NULLIF",
            "CAST", "CONVERT", "NOW", "CURRENT_DATE", "CURRENT_TIME",
            "CURRENT_TIMESTAMP", "DATE", "DATEADD", "DATEDIFF", "SUBSTRING",
            "TRIM", "UPPER", "LOWER", "LENGTH", "LEN", "CONCAT",
            "ISNULL", "IFNULL", "NVL", "DECODE", "ROUND", "FLOOR", "CEIL",
...
pub fn detect_language(code: &str) -> Language {
    ...
    if code.contains("fn ") && (code.contains("let ") || code.contains("impl ") || code.contains("-> ")) {
...
pub fn highlight_line(
    ...
    while i < len {
        ...
        if ch.is_ascii_digit() {
            ...
            let is_start = prev == ' ' || prev == '\t' || prev == '(' || prev == '['
            ...
            if is_start {
                ...
                if ch == '0' && i + 1 < len && (chars[i + 1] == 'x' || chars[i + 1] == 'X') {
                    ...
                    if matches!(lang, Language::Rust) && i < len
